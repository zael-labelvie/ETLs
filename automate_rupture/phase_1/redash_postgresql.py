...
data_Picking = pd.read_csv(
    'https://redash-ro-live.bringo.ro/api/queries/257/results.csv?api_key=oKpvHFlf3WvvoxXTN4svazLMNm2dV0BpWHoVU7My')
...
Data_Product = pd.read_csv(
    'https://redash-ro-live.bringo.ro/api/queries/2499/results.csv?api_key=Gcrjf7lXeowl8bLQvYw0jjuFMjrMmv7ho6p58lob')
df_Product = pd.DataFrame(Data_Product)


################################## Get Data From Redash Financial Report Table #################################
data_assortiment = pd.read_excel(
    'C:/Users/LAMIA/Desktop/Assortiement/Assortiment Bringo 31_08_2022.xlsx')
...
def Sx(x):
    if x['picker_selected'] == 'nan':
        return x['order_start']

    else:
        return x['picker_selected']
...
def delivery_date_end(x):
    if x['delivery_date_end'] == 'nan':
        return x['picking_finish_at']
    else:
        return x['delivery_date_end']

df_Financial['delivery_date_end'] = df_Financial.apply(lambda x: delivery_date_end(x), axis=1)
...
#df_Picking['picker_selected'] = df_Picking.apply(lambda x : Sx(x),axis=1)
#df_Picking['picked'] = df_Picking.apply(lambda x : Kx(x),axis=1)
#df_Picking['store_paid'] = df_Picking.apply(lambda x : Ix(x),axis=1)
...
conn_string = 'postgresql://postgres:zael123456@localhost/Bringo'
...
requete_vision_product = """select distinct DP.order_date ,DOI.product_name, DOI.unit_price,DOI.quantity,DOI.units_total, DA."Code interne",DA."Département", DA."FAMILLE", DA."RAYON", DA."SOUS FAMILLE", DA."SSFAM",DPR.picker_state, RD.number,RD.order_state, DP.time_select_picker, DP.time_picking, RD.delivery_date_end ,RD.store_internal_name,DP.user_picker         
into Vision_Product  
...
and RD.order_state not like 'canceled'
...
requete_vision_commande = """select vision_product.number, to_date(to_char(vision_product.order_date, 'DD/MM/YYYY'), 'DD/MM/YYYY'), sum(Quantity_Replaced_Tag) as Quantity_Replaced, sum(Quantity_Unfound_Tag) as Quantity_Unfound ,sum(Quantity_found_Tag) as Quantity_found ,CASE
...
from vision_product
group by to_date(to_char(vision_product.order_date, 'DD/MM/YYYY'), 'DD/MM/YYYY'), vision_product.number 
